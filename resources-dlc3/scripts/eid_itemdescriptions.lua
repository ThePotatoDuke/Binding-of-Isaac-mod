if EID then
    ...
    EID:addCollectible(Isaac.GetItemIdByName("Wind-Up Key"),
        "Fire in a circling motion to wind up.#Stopping releases a spiral of tears in the opposite direction.# Pressing the wrong button resets the winding#\1 Tears from the spiral deal Isaac's dmg")
    EID:addCollectible(Isaac.GetItemIdByName("Critical Hit"),
        "\2 0.8 range multiplier# Tears get larger right before hitting the ground# \1 If the tear hits an enemy during this time, it deals Isaac's damage x 1.6")
    ...
    EID:addCollectible(Isaac.GetItemIdByName("Malediction"),
        "Isaac fires a special tear every 3 tears while holding the item# If these tears hit an enemy, the enemy is marked# Using the item deals damage to the marked enemeis#\1 The damage dealt increases with the number of marked enemies (1.8 x dmg x marked enemy count)")
end
